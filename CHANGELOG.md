...
## [Unreleased]
### Added
- Документация: `docs/DOMAIN_MIGRATION_orpheus.click.md` — план разделения доменов (сайт / API / обновления) с сохранением старого домена для текущих клиентов.
- Клиент: подготовка миграции домена — список API/WS хостов (новый → legacy) и генерация URL с поддержкой fallback.
- Обновления: fallback-запрос `check-update` по хостам + корректная обработка абсолютных `download_url` (для `update.orpheus.click`).
- Сообщения/звонки: вынесена обработка входящих WS сообщений в тестируемый сервис `IncomingMessageHandler`.
- Звонки: выделен буфер входящих сигналов/ICE (`IncomingCallBuffer`) для устранения race condition.
- Тесты: добавлены контрактные тесты для обработки входящих сообщений/звонков и протокола исходящих пакетов (WS + HTTP fallback для `hang-up`/`call-rejected`).
- Документация тестов: добавлен единый вход `docs/testing/README.md`.
- История обновлений: загрузка “Что нового” с публичного API (если доступно), с fallback на встроенный список при офлайне.
- Клиент: дисклеймер о бета-версии на входе (кнопка “Я понял(а)” + опция “Больше не показывать”).
- Контакты/чат: presence-статусы (в сети / не в сети) по WebSocket.
