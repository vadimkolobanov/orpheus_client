...
## 11) Телеметрия (временный режим разработки)
- Источник: `DebugLogger` + перехват `debugPrint` + `FlutterError`.
- Отправка: HTTP батчи на `/api/logs/batch` с заголовком `X-Pubkey`.
- В фоне (FCM): базовые события также отправляются в телеметрию.
- Назначение: полный цикл жизни клиента для диагностики.
