...
**Где**: `DebugLogger` + полная телеметрия (HTTP/WS payload, фоновые события).

**Риск**:
Полная телеметрия может раскрыть токены/метаданные и технические контексты.

**Рекомендация**:
- строгий TTL для телеметрии и ограничение доступа;
- redaction (маскирование токенов/идентификаторов);
- отключать полную телеметрию в прод‑режиме.
