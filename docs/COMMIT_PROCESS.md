...
## Release notes / "Что нового" (единый публичный changelog)

- Пользовательский "Что нового" больше **не ведём** в клиенте (`AppConfig.changelogData` — legacy).
- Единый источник release notes: `OPHEUS_ADMIN` → **"Версии"** (`app_versions`).
- При пользовательских изменениях не забудьте обновить запись версии в админке (это то, что увидят пользователи в клиенте/на сайте).
