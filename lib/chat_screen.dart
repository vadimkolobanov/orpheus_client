...
class _ChatScreenState extends State<ChatScreen> {
  // ... (все методы до build остаются без изменений)
  final _messageController = TextEditingController();
  List<ChatMessage> _chatHistory = [];
  late StreamSubscription _messageUpdateSubscription;
  @override
  void initState() { super.initState(); _loadChatHistory();
  _messageUpdateSubscription = messageUpdateController.stream.listen((senderKey) {
    if (senderKey == widget.contact.publicKey) { _loadChatHistory(); }
  });
  }
  ...
  @override
  void dispose() { _messageUpdateSubscription.cancel(); _messageController.dispose(); super.dispose(); }
  void _sendMessage() async {
    ...
    try {
      ...
      websocketService.sendMessage(widget.contact.publicKey, payload);
    } catch (e) { print("Ошибка отправки: $e"); }
  ...
  void _showClearHistoryDialog() {
    showDialog(context: context, builder: (context) => AlertDialog(
      ...
      actions: [
        ...
        TextButton(child: const Text('Удалить', style: TextStyle(color: Colors.red)),
            onPressed: () async { await DatabaseService.instance.clearChatHistory(widget.contact.publicKey);
            Navigator.pop(context); _loadChatHistory();
            }),
      ],
    ));
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text(widget.contact.name), actions: [
        IconButton(icon: const Icon(Icons.delete_sweep_outlined),
          tooltip: 'Очистить историю чата', onPressed: _showClearHistoryDialog,
        )
      ]),
      body: Column(children: [
        Expanded(
          child: ListView.builder(
            reverse: true, // Начинаем с конца списка, как в мессенджерах
            padding: const EdgeInsets.symmetric(horizontal: 16.0, vertical: 8.0),
            itemCount: _chatHistory.length,
            itemBuilder: (context, index) {
              // Так как список перевернут, идем с конца
        ...
        Container(
          ...
          child: SafeArea(
            child: Row(children: [
              Expanded(
                child: TextField(
                  ...
                  decoration: const InputDecoration(
                    hintText: 'Сообщение...',
                    // Убираем внутренний фон, используем цвет из inputDecorationTheme
                    filled: false,
                  ),
              ...
              IconButton.filled( // Делаем кнопку более заметной
